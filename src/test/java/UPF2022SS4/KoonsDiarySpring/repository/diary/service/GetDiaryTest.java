...
//                .diaryImageList(diaryImageList)
        ...
        // when
        DefaultResponse response1 = diaryService.postDiary(postDiaryRequest);
        System.out.println("postDiaryRequest = " + response1);


        GetDiaryListRequest getDiaryListRequest = new GetDiaryListRequest(user.getId());
        DefaultResponse diaryList = diaryService.getDiaryList(getDiaryListRequest);


        GetDiaryRequest getDiary= new GetDiaryRequest(9L);

        DefaultResponse response2 = diaryService.getDiary(getDiary);

        // then
        System.out.println("동작 결과 = " + response2);
    }
}
